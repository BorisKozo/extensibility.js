...
module.exports = function (file, options) {
  if (!file) {
    throw new PluginError('gulp-compile-sdv-modules', 'Missing file option for gulp-compile-sdv-modules');
  }
  options = options || {};
  ...
  if (typeof file === 'string') {
    ...
    throw new PluginError('gulp-compile-sdv-modules', 'Missing path in file options for gulp-compile-sdv-modules');
  }
  ...
  function bufferContents(file, enc, cb) {
    ...
    if (file.isStream()) {
      this.emit('error', new PluginError('gulp-compile-sdv-modules', 'Streaming not supported'));
      cb();
      return;
    }
