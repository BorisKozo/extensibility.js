describe('Topological Sort', function () {
    ...
    describe('formSortClusters', function () {
        ...
        describe('direct dependency', function () {
            it('should create one cluster for single direct dependency >', function () {
                ...
                addins.push(new subdivision.Addin({id: '2', order: '>1'}));
                ...
                addins = [];
                addins.push(new subdivision.Addin({id: '2', order: '>1'}));
                addins.push(new subdivision.Addin({id: '3', order: '>2'}));
                addins.push(new subdivision.Addin({id: '1', order: 0}));
                var clusters = subdivision.utils.topologicalSort._formSortClusters(addins);
                expect(clusters.length).to.be.equal(1);
            ...
            it('should create one cluster for mixed direct dependencies', function () {
                var addins = [];
                addins.push(new subdivision.Addin({id: '2', order: '<1'}));
                addins.push(new subdivision.Addin({id: '3', order: '>1'}));
                addins.push(new subdivision.Addin({id: '1', order: 0}));
                var clusters = subdivision.utils.topologicalSort._formSortClusters(addins);
                expect(clusters.length).to.be.equal(1);
            ...
            it('should create one cluster for mutual direct dependencies', function () {
                var addins = [];
                addins.push(new subdivision.Addin({id: '1', order: '<2'}));
                addins.push(new subdivision.Addin({id: '2', order: '>1'}));
                var clusters = subdivision.utils.topologicalSort._formSortClusters(addins);
                expect(clusters.length).to.be.equal(1);
                expect(verifyClusterOrder(clusters[0], ['1', '2'])).to.be.true;
            ...
            it('should throw if direct dependencies collide', function () {
                ...
                addins.push(new subdivision.Addin({id: '1', order: 0}));
                expect(function () {
                    subdivision.utils.topologicalSort._formSortClusters(addins);
        ...
        describe('indirect dependency', function () {
            ...
            it('should create three clusters of indirect dependency', function () {
                ...
                addins.push(new subdivision.Addin({id: '3', order: '>>1'}));
                addins.push(new subdivision.Addin({id: '1', order: 0}));
                addins.push(new subdivision.Addin({id: '2', order: '>>3'}));
                var clusters = subdivision.utils.topologicalSort._formSortClusters(addins);
                expect(clusters.length).to.be.equal(3);
                expect(verifyClusterOrder(clusters[0], ['2'])).to.be.true;
                expect(verifyClusterOrder(clusters[1], ['1'])).to.be.true;
        ...
        describe('multiple indirect dependencies', function () {
            it('should order correctly for multiple indirect dependencies', function () {
                ...
                addins.push(new subdivision.Addin({id: '1', order: 0}));
                addins.push(new subdivision.Addin({id: '3', order: '>>1'}));
                addins.push(new subdivision.Addin({id: '2', order: '>>1,>>3'}));
                var clusters = subdivision.utils.topologicalSort._formSortClusters(addins);
                expect(clusters.length).to.be.equal(3);
                expect(verifyClusterOrder(clusters[0], ['2'])).to.be.true;
        ...
        describe('general errors', function () {
            it('should throw if referencing an not existing id', function () {
                var addins = [];
                addins.push(new subdivision.Addin({id: '1', order: 0}));
                addins.push(new subdivision.Addin({id: '2', order: '>33'}));
                expect(function () {
                    subdivision.utils.topologicalSort._formSortClusters(addins);
            ...
            it('should throw if order is in an incorrect format', function () {
                var addins = [];
                addins.push(new subdivision.Addin({id: '1', order: true}));
                expect(function () {
                    subdivision.utils.topologicalSort._formSortClusters(addins);
                }).to.throw('order must begin with <, <<, >, >> or be a number');

                addins = [];
                addins.push(new subdivision.Addin({id: '1', order: 'moo'}));
                expect(function () {
                    subdivision.utils.topologicalSort._formSortClusters(addins);
                }).to.throw('order must begin with <, <<, >, >> or be a number');
            });
    ...
    describe('topologicalSort', function () {
        ...
        it('should properly sort mixed ordered addins', function () {
            ...
            addins.push(new subdivision.Addin({id: '2', order: '>1'}));
            addins.push(new subdivision.Addin({id: '3', order: '>>2'}));
            addins.push(new subdivision.Addin({id: '4', order: 20}));
            addins.push(new subdivision.Addin({id: '5', order: '<4'}));
            var result = subdivision.utils.topologicalSort(addins);
            expect(result.length).to.be.equal(5);
        ...
        it('should throw if circular dependency is found', function () {
            var addins = [];
            addins.push(new subdivision.Addin({id: '1', order: '>>3'}));
            addins.push(new subdivision.Addin({id: '2', order: '>>1'}));
            addins.push(new subdivision.Addin({id: '3', order: '>>2'}));
            expect(function () {
                subdivision.utils.topologicalSort(addins);
        ...
        it('should properly sort multiple indirect dependencies', function () {
            ...
            addins.push(new subdivision.Addin({id: '1', order: 0}));
            addins.push(new subdivision.Addin({id: '2', order: '<<1,<<3'}));
            addins.push(new subdivision.Addin({id: '3', order: '>>2,<<1'}));
            var result = subdivision.utils.topologicalSort(addins);
            expect(result.length).to.be.equal(3);
        ...
        it('should throw if there is a circular dependency in mixed indirect after and direct before',function(){
            ...
            addins.push(new subdivision.Addin({id: '1', order: 0}));
            addins.push(new subdivision.Addin({id: '2', order: '>>3,<<0,>1'}));
            addins.push(new subdivision.Addin({id: '3', order: 20}));
            addins.push(new subdivision.Addin({id: '4', order: '<<2,>>0,<3'}));
            expect(function () {
                subdivision.utils.topologicalSort(addins);
            }).to.throw('Circular dependency detected in topological sort');
        });
